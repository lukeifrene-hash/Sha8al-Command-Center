...
| **`prepare M<N> <tier>`** *or* **`prepare M<N> all`** *or* **`prepare T<id>`** | Spawns Explorer + Researcher sub-agents; writes `docs/prompts/M<N>/<task_id>.md`; stores that path as `builder_prompt`; no code written. `all` is a convenience wrapper for every non-small task in the milestone. | Before building medium or large/architectural work |
| **`build T<id>`** *or* **`build M<N> medium`** | Executes prepared task(s) from `builder_prompt` when present; auditor runs after each build; auto-approves on pass (backend lanes only) | After prepare, when enrichment is ready |
...
**Context source:** tracker task brief + builder prompt markdown (`docs/prompts/M<N>/<task_id>.md`) + Explorer findings (codebase scan) + Researcher findings (external docs).
...
5. Orchestrator writes `docs/prompts/M<N>/<task_id>.md` as the canonical builder brief
6. Calls `enrich_task` with `builder_prompt` pointing to that markdown file and with updated context/reference/constraint fields
7. Orchestrator throttles to ~5 parallel pairs to stay rate-limit sane
8. When done, every medium task in M<N> has a builder prompt file and the tracker points at it
...
2. For each, calls `claim_next_task(tier=medium)` + reads `builder_prompt` if present (fallback: tracker prompt)
...
When a wave contains multiple medium tasks with no file collisions and no dependency edge between them, the orchestrator should prefer parallel Agent subagents by default. Serialization is reserved for colliding tasks or tasks that depend on each other.
...
**Context source:** tracker task brief + builder prompt markdown under `docs/prompts/M<N>/` (if present) + milestone brief (if one exists) + Explorer findings (broader codebase map) + Researcher findings (deeper external research).
...
### Prepare all non-small — convenience wrapper, same tier semantics

**Context source:** unchanged from the underlying task tier. `prepare M<N> all` does not invent a new preparation style; it batches together the existing `medium` and `large` / `architectural` styles under one operator command.

**Prepare flow** (`prepare M<N> all`):
1. Orchestrator calls `bulk_prepare(milestone_id)` with no tier filter — MCP returns every non-small task in the milestone that still needs preparation.
2. Split the batch by complexity:
   - `medium` tasks use the normal batched prepare flow
   - `large` / `architectural` tasks use the normal deep per-task prepare flow
3. Prepare the `medium` tasks first, because they parallelize well and usually define or confirm local patterns quickly.
4. Prepare the `large` / `architectural` tasks second, preserving deeper exploration scope and lower concurrency.
5. If any `large` / `architectural` task would benefit from a milestone brief, create or refresh it once before finalizing those deep-prep prompts.
6. Write `builder_prompt` files and tracker enrichment exactly as in the underlying tier-specific flows.

**The rule:** `all` is a convenience wrapper, not a semantic flattening. Large / architectural tasks must not be reduced to medium-style 1–3 file exploration just because they were included in a milestone-wide prepare run.
