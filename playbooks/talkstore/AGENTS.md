...
| `prepare M<N> <tier>` **or** `prepare T<id>` | Call `bulk_prepare` (batch) or `get_task_context` (single). For each target task, spawn Explorer + Researcher sub-agents in parallel; they write findings back via `enrich_task`. Does NOT execute code. | `.claude/commands/prepare.md` |
